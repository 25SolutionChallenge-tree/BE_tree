...
public interface DiaryRepository extends JpaRepository<Diary, Long> {
    ...
    // 특정 연도와 월의 일기 조회
    @Query("SELECT d FROM Diary d WHERE d.userId = :userId AND YEAR(d.createdAt) = :year AND MONTH(d.createdAt) = :month ORDER BY d.createdAt")
    ...
    @Query("SELECT d FROM Diary d WHERE d.userId = :userId AND DATE(d.createdAt) = DATE(:date) ORDER BY d.qtype")
    List<Diary> findByUserIdAndDate(@Param("userId") Long userId, @Param("date") LocalDateTime date);

    // 특정 연도와 월에 사용자가 일기를 작성한 날짜 목록 조회
    @Query("SELECT DISTINCT DATE(d.createdAt) FROM Diary d WHERE d.userId = :userId AND YEAR(d.createdAt) = :year AND MONTH(d.createdAt) = :month ORDER BY DATE(d.createdAt)")
